...
struct thread_t {
  ...
  /* program counter */
  md_addr_t pred_PC;
  md_addr_t recover_PC; /* when is it used? */
  ...
  BITMAP_TYPE(MD_NUM_CREGS, use_spec_C);
  ...
  /* simulated memory, different address space per thread */
  struct mem_t *mem;
  ...
  int mcount; /* M-COUNT scheduling policy counter */
    ...
    int thread_id) /* thread ID of cache block */
{
  ...
  if (cache_dl2)
  {
    ...
    lat = cache_access(cache_dl2, cmd, baddr, NULL, bsize,
        /* now */now, /* pudata */NULL, /* repl addr */NULL, thread_id);
    if (cmd == Read)
      return lat;
    else
    {
...
sim_init(void)
{
  ...
  for (th=0; th<nthread; ++th) {
    ...
    regs_init(&(thread[th].regs));

    /* allocate and initialize memory space */
    thread[th].mem = mem_create("mem");
    mem_init(thread[th].mem);
  }
}
...
struct RUU_station {
  ...
  int thread_id; /* thread ID for multithreading */
};
...
/* static struct RUU_station *RUU;		/1* register update unit *1/ */
/* static int RUU_head, RUU_tail;		/1* RUU head and tail pointers *1/ */
/* static int RUU_num;			/1* num entries currently in RUU *1/ */
...
ruu_init(void)
{
  int th;
  for (th=0; th<nthread; ++th) {
    thread[th].RUU = calloc(RUU_size, sizeof(struct RUU_station));
    if (!thread[th].RUU)
      fatal("out of virtual memory");

    thread[th].RUU_num = 0;
    thread[th].RUU_head = thread[th].RUU_tail = 0;
    RUU_count = 0;
    RUU_fcount = 0;
  }
}
...
ruu_dump(FILE *stream)				/* output stream */
{
  ...
  for (th=0; th<nthread; ++th) {
    fprintf(stream, "** RUU state of thread %d**\n", th);
    fprintf(stream, "RUU_head: %d, RUU_tail: %d\n", thread[th].RUU_head, thread[th].RUU_tail);
    fprintf(stream, "RUU_num: %d\n", thread[th].RUU_num);
    num = thread[th].RUU_num;
    head = thread[th].RUU_head;
    while (num)
    {
      rs = &(thread[th].RUU[head]);
      ruu_dumpent(rs, rs - thread[th].RUU, stream, /* header */TRUE);
 ...
 */
/* static struct RUU_station *LSQ;         /1* load/store queue *1/ */
/* static int LSQ_head, LSQ_tail;          /1* LSQ head and tail pointers *1/ */
/* static int LSQ_num;                     /1* num entries currently in LSQ *1/ */
...
lsq_init(void)
{
  int th;
  for (th=0; th<nthread; ++th) {
    thread[th].LSQ = calloc(LSQ_size, sizeof(struct RUU_station));
    if (!thread[th].LSQ)
      fatal("out of virtual memory");

    thread[th].LSQ_num = 0;
    thread[th].LSQ_head = thread[th].LSQ_tail = 0;
  }
  LSQ_count = 0;
  LSQ_fcount = 0;
}
...
lsq_dump(FILE *stream)				/* output stream */
{
  ...
  for (th=0; th<nthread; ++th) {
    fprintf(stream, "** LSQ state of thread %d **\n", th);
    fprintf(stream, "LSQ_head: %d, LSQ_tail: %d\n", thread[th].LSQ_head, thread[th].LSQ_tail);
    fprintf(stream, "LSQ_num: %d\n", thread[th].LSQ_num);

    num = thread[th].LSQ_num;
    head = thread[th].LSQ_head;
    while (num)
    {
      rs = &(thread[th].LSQ[head]);
      ruu_dumpent(rs, rs - thread[th].LSQ, stream, /* header */TRUE);
  ...
  for (ev = event_queue; ev != NULL; ev = ev->next)
  {
    ...
    if (RSLINK_VALID(ev))
    {
      ...
      fprintf(stream, "idx: %2d: @ %.0f\n",
          (int)(rs - (rs->in_LSQ ? thread[rs->thread_id].LSQ : thread[rs->thread_id].RUU)), (double)ev->x.when);
      ruu_dumpent(rs, rs - (rs->in_LSQ ? thread[rs->thread_id].LSQ : thread[rs->thread_id].RUU),
          stream, /* !header */FALSE);
    }
  }
}
...
cv_init(void)
{
  ...
  for (th=0; th<nthread; ++th) {
    ...
    {
      curr_th->create_vector[i] = CVLINK_NULL;
      curr_th->create_vector_rt[i] = 0;
      curr_th->spec_create_vector[i] = CVLINK_NULL;
      curr_th->spec_create_vector_rt[i] = 0;
    }

    /* all create vector entries are non-speculative */
    BITMAP_CLEAR_MAP(curr_th->use_spec_cv, CV_BMAP_SZ);
  }
}
  ...
  for (i=0; i < MD_TOTAL_REGS; i++)
  {
    ...
    if (!ent.rs)
      ...
      fprintf(stream, "[cv%02d]: from %s, idx: %d\n",
          ...
          (int)(ent.rs - (ent.rs->in_LSQ ? thread[ent.rs->thread_id].LSQ : thread[ent.rs->thread_id].RUU)));
  }
}
...
/* recover processor microarchitecture state back to point of the
  ...
  static void
ruu_recover(int thread_id, int branch_index)			/* index of mis-pred branch */
{
  struct thread_t *curr_th = &(thread[thread_id]);
  int i, RUU_index = curr_th->RUU_tail, LSQ_index = curr_th->LSQ_tail;
  int RUU_prev_tail = curr_th->RUU_tail, LSQ_prev_tail = curr_th->LSQ_tail;
  ...
  while (RUU_index != branch_index)
  {
    ...
    if (curr_th->RUU[RUU_index].ea_comp)
    {
      ...
      for (i=0; i<MAX_ODEPS; i++)
      {
        RSLINK_FREE_LIST(curr_th->LSQ[LSQ_index].odep_list[i]);
        /* blow away the consuming op list */
        curr_th->LSQ[LSQ_index].odep_list[i] = NULL;
      }

      /* squash this LSQ entry */
      curr_th->LSQ[LSQ_index].tag++;

      /* indicate in pipetrace that this instruction was squashed */
      ptrace_endinst(curr_th->LSQ[LSQ_index].ptrace_seq);
    ...
    for (i=0; i<MAX_ODEPS; i++)
    {
      RSLINK_FREE_LIST(curr_th->RUU[RUU_index].odep_list[i]);
      /* blow away the consuming op list */
      curr_th->RUU[RUU_index].odep_list[i] = NULL;
    }

    /* squash this RUU entry */
    curr_th->RUU[RUU_index].tag++;

    /* indicate in pipetrace that this instruction was squashed */
    ptrace_endinst(curr_th->RUU[RUU_index].ptrace_seq);
  ...
  curr_th->RUU_tail = RUU_prev_tail;
  curr_th->LSQ_tail = LSQ_prev_tail;
...
/* forward declarations */
static void tracer_recover(int thread_id);
    ...
    for (i=0; i<MAX_ODEPS; i++)
    {
      if (rs->onames[i] != NA)
      {
        ...
        if (rs->spec_mode)
        {
          ...
          link = curr_th->spec_create_vector[rs->onames[i]];
          if (/* !NULL */link.rs
              && /* refs RS */(link.rs == rs && link.odep_num == i))
          {
            /* the result can now be read from a physical register,
               indicate this as so */
            curr_th->spec_create_vector[rs->onames[i]] = CVLINK_NULL;
            curr_th->spec_create_vector_rt[rs->onames[i]] = sim_cycle;
          }
        ...
        else
        {
          /* update the non-speculative create vector, future
             ...
             reg file */
          link = curr_th->create_vector[rs->onames[i]];
          if (/* !NULL */link.rs
              && /* refs RS */(link.rs == rs && link.odep_num == i))
          {
            /* the result can now be read from a physical register,
               indicate this as so */
            curr_th->create_vector[rs->onames[i]] = CVLINK_NULL;
            curr_th->create_vector_rt[rs->onames[i]] = sim_cycle;
          }
          /* else, creator invalidated or there is another creator */
        }
...
lsq_refresh(void)
{
  ...
  for (th=0; th<nthread; ++th) {
    struct thread_t *curr_th = &(thread[th]);
    for (i=0, index=curr_th->LSQ_head, n_std_unknowns=0;
        i < curr_th->LSQ_num;
        i++, index=(index + 1) % LSQ_size)
    {
      ...
      if (/* store? */
          (MD_OP_FLAGS(curr_th->LSQ[index].op) & (F_MEM|F_STORE)) == (F_MEM|F_STORE))
      {
        if (!STORE_ADDR_READY(&curr_th->LSQ[index]) && !perfect_disambig)
        {
          /* FIXME: a later STD + STD known could hide the STA unknown */
          /* STA unknown, blocks all later loads, stop search */
          break;
        }
        else if (!OPERANDS_READY(&curr_th->LSQ[index]) ||
            (!STORE_ADDR_READY(&curr_th->LSQ[index]) && perfect_disambig))
        {
          ...
          std_unknowns[n_std_unknowns++] = curr_th->LSQ[index].addr;
        }
        else /* STORE_ADDR_READY() && OPERANDS_READY() */
        {
          ...
          {
            if (std_unknowns[j] == /* STA/STD known */curr_th->LSQ[index].addr)
      ...
      if (/* load? */
          ((MD_OP_FLAGS(curr_th->LSQ[index].op) & (F_MEM|F_LOAD)) == (F_MEM|F_LOAD))
          && /* queued? */!curr_th->LSQ[index].queued
          && /* waiting? */!curr_th->LSQ[index].issued
          && /* completed? */!curr_th->LSQ[index].completed
          && /* regs ready? */OPERANDS_READY(&curr_th->LSQ[index]))
      {
        ...
        for (j=0; j<n_std_unknowns; j++)
        {
          ...
          if (std_unknowns[j] == curr_th->LSQ[index].addr)
            break;
        }
        if (j == n_std_unknowns)
        {
          /* no STA or STD unknown conflicts, put load on ready queue */
          readyq_enqueue(&curr_th->LSQ[index]);
        }
      }
    }
  }
                  ...
                  == (F_MEM|F_LOAD)))
            {
              ...
              if (i != curr_th->LSQ_head)
              {
                for (;;)
                {
                  ...
                  if ((MD_OP_FLAGS(curr_th->LSQ[i].op) & F_STORE)
                      && (curr_th->LSQ[i].addr == rs->addr))
                  {
                    /* hit in the LSQ */
                    load_lat = 1;
                    break;
              ...
              if (dtlb && MD_VALID_ADDR(rs->addr))
              {
                ...
                tlb_lat =
                  cache_access(dtlb, Read, (rs->addr & ~3),
                      NULL, 4, sim_cycle, NULL, NULL, rs->thread_id);
                if (tlb_lat > 1)
                  events |= PEV_TLBMISS;
...
rspec_dump(FILE *stream)			/* output stream */
{
  ...
  for (th=0; th<nthread; ++th) {
    ...
    for (i=0; i < MD_NUM_IREGS; i++)
    {
      ...
      {
        md_print_ireg(curr_th->spec_regs.regs_R, i, stream);
        fprintf(stream, "\n");
      }
    }
    ...
    {
      if (BITMAP_SET_P(curr_th->use_spec_F, F_BMAP_SZ, i))
      {
        md_print_fpreg(curr_th->spec_regs.regs_F, i, stream);
        fprintf(stream, "\n");
      }
    }
    ...
    {
      if (BITMAP_SET_P(curr_th->use_spec_C, C_BMAP_SZ, i))
      {
        md_print_creg(curr_th->spec_regs.regs_C, i, stream);
        fprintf(stream, "\n");
      }
    }
  }
...
/* static struct fetch_rec *fetch_data;	/1* IFETCH -> DISPATCH inst queue *1/ */
/* static int fetch_num;			/1* num entries in IF -> DIS queue *1/ */
/* static int fetch_tail, fetch_head;	/1* head and tail pointers of queue *1/ */
...
tracer_recover(int thread_id)
{
  ...
  BITMAP_CLEAR_MAP(curr_th->use_spec_F, F_BMAP_SZ);
  BITMAP_CLEAR_MAP(curr_th->use_spec_C, C_BMAP_SZ);
  ...
  if (ptrace_active)
  {
    while (curr_th->fetch_num != 0)
    {
      /* squash the next instruction from the IFETCH -> DISPATCH queue */
      ptrace_endinst(curr_th->fetch_data[curr_th->fetch_head].ptrace_seq);

      /* consume instruction from IFETCH -> DISPATCH queue */
      curr_th->fetch_head = (curr_th->fetch_head+1) & (ruu_ifq_size - 1);
      curr_th->fetch_num--;
    }
  }
  ...
  /* reset IFETCH state */
  curr_th->fetch_num = 0;
  curr_th->fetch_tail = curr_th->fetch_head = 0;
  curr_th->fetch_pred_PC = curr_th->fetch_regs_PC = curr_th->recover_PC;
}
...
#define SET_NPC(EXPR)           (curr_th->regs.regs_NPC = (EXPR))
...
#define CPC                     (curr_th->regs.regs_PC)
#define SET_CPC(EXPR)           (curr_th->regs.regs_PC = (EXPR))
...
#define SET_FPR_D(N,EXPR)       (curr_th->spec_mode				\
    ? ((curr_th->spec_regs.regs_F.d[(N) >> 1] = (EXPR)),	\
      BITMAP_SET(curr_th->use_spec_F,F_BMAP_SZ,((N)&~1)),\
      curr_th->spec_regs.regs_F.d[(N) >> 1])		\
      : (curr_th->regs.regs_F.d[(N) >> 1] = (EXPR)))
...
simoo_reg_obj(struct regs_t *xregs,		/* registers to access */
    ...
    struct eval_value_t *val)		/* input, output */
{
  struct thread_t *curr_th = &(thread[xregs->thread_id]);
  ...
  switch (rt)
  {
    ...
    case rt_PC:
      ...
      {
        val->type = et_addr;
        val->value.as_addr = curr_th->regs.regs_PC;
      }
      else
        curr_th->regs.regs_PC = eval_as_addr(*val);
      break;
    ...
    case rt_NPC:
      ...
      {
        val->type = et_addr;
        val->value.as_addr = curr_th->regs.regs_NPC;
      }
      else
        curr_th->regs.regs_NPC = eval_as_addr(*val);
      break;
...
ruu_dispatch(void)
{
      ...
      && (ruu_include_spec || !curr_th->spec_mode))
  {
    ...
    inst = curr_th->fetch_data[curr_th->fetch_head].IR;
    curr_th->regs.regs_PC = curr_th->fetch_data[curr_th->fetch_head].regs_PC;
    curr_th->pred_PC = curr_th->fetch_data[curr_th->fetch_head].pred_PC;
    dir_update_ptr = &(curr_th->fetch_data[curr_th->fetch_head].dir_update);
    stack_recover_idx = curr_th->fetch_data[curr_th->fetch_head].stack_recover_idx;
    pseq = curr_th->fetch_data[curr_th->fetch_head].ptrace_seq;
    ...
    curr_th->regs.regs_R[MD_REG_ZERO] = 0; curr_th->spec_regs.regs_R[MD_REG_ZERO] = 0;
#ifdef TARGET_ALPHA
...
#include "machine.def"
    ...
    br_taken = (curr_th->regs.regs_NPC != (curr_th->regs.regs_PC + sizeof(md_inst_t)));
    br_pred_taken = (curr_th->pred_PC != (curr_th->regs.regs_PC + sizeof(md_inst_t)));

    /* Check for perfection prediction inconsistencies. */
    if (curr_th->pred_PC != curr_th->regs.regs_NPC && pred_perfect)
    {
      curr_th->pred_PC = curr_th->regs.regs_NPC;
      curr_th->fetch_pred_PC = curr_th->fetch_regs_PC = curr_th->regs.regs_NPC;
     ...
     * the PC values and do a fetch squash. */
        ...
        && target_PC != curr_th->pred_PC && br_pred_taken)
    {
      misfetch_count++;
      recovery_count++;
      ...
      if (mf_compat) {
        ...
        curr_th->fetch_pred_PC = curr_th->fetch_regs_PC = target_PC;
        if (br_taken) {
          fetch_redirected = TRUE;
          misfetch_only_count++;
        }
    ...
    if (op != MD_NOP_OP)
    {
      ...
      rs = &(curr_th->RUU[curr_th->RUU_tail]);
      rs->thread_id = th_id;
      rs->slip = sim_cycle - 1;
      rs->IR = inst;
      rs->op = op;
      rs->PC = curr_th->regs.regs_PC;
      rs->next_PC = curr_th->regs.regs_NPC; rs->pred_PC = curr_th->pred_PC;
      rs->in_LSQ = FALSE;
      rs->ea_comp = FALSE;
      ...
      if (MD_OP_FLAGS(op) & F_MEM)
      {
        ...
        lsq = &(curr_th->LSQ[curr_th->LSQ_tail]);
        lsq->thread_id = th_id;
        lsq->slip = sim_cycle - 1;
        lsq->IR = inst;
        lsq->op = op;
        lsq->PC = curr_th->regs.regs_PC;
        lsq->next_PC = curr_th->regs.regs_NPC; lsq->pred_PC = curr_th->pred_PC;
        lsq->in_LSQ = TRUE;
        lsq->ea_comp = FALSE;
    ...
    if (!curr_th->spec_mode)
    {
      ...
      if (MD_OP_FLAGS(op) & F_CTRL)
      {
        ...
        {
          bpred_update(pred,
              /* branch address */curr_th->regs.regs_PC,
              /* actual target address */curr_th->regs.regs_NPC,
              /* taken? */curr_th->regs.regs_NPC != (curr_th->regs.regs_PC +
                sizeof(md_inst_t)),
              /* pred taken? */curr_th->pred_PC != (curr_th->regs.regs_PC +
                sizeof(md_inst_t)),
              /* correct pred? */curr_th->pred_PC == curr_th->regs.regs_NPC,
              /* opcode */op,
    ...
    ptrace_newstage(pseq, PST_DISPATCH,
        (curr_th->pred_PC != curr_th->regs.regs_NPC) ? PEV_MPOCCURED : 0);
    if (op == MD_NOP_OP)
    {
      /* end of the line */
      ptrace_endinst(pseq);
    ...
    for (i=0; i<pcstat_nelt; i++)
    {
      ...
      if (delta != 0)
      {
        stat_add_samples(pcstat_sdists[i], curr_th->regs.regs_PC, delta);
    ...
    made_check = TRUE;
    if (dlite_check_break(curr_th->pred_PC,
          is_write ? ACCESS_WRITE : ACCESS_READ,
          addr, sim_num_insn, sim_cycle))
      dlite_main(curr_th->regs.regs_PC, curr_th->pred_PC, sim_cycle, &curr_th->regs, curr_th->mem);
  }
...
fetch_dump(FILE *stream)			/* output stream */
{
  ...
  for (th=0; th<nthread; ++th) {
    ...
    fprintf(stream, "** fetch queue contents of thread %d **\n", th);
    fprintf(stream, "fetch_num: %d\n", curr_th->fetch_num);
    fprintf(stream, "fetch_head: %d, fetch_tail: %d\n",
        curr_th->fetch_head, curr_th->fetch_tail);

    num = curr_th->fetch_num;
    head = curr_th->fetch_head;
    while (num)
    {
      fprintf(stream, "idx: %2d: inst: `", head);
      md_print_insn(curr_th->fetch_data[head].IR, curr_th->fetch_data[head].regs_PC, stream);
      fprintf(stream, "'\n");
      myfprintf(stream, "         regs_PC: 0x%08p, pred_PC: 0x%08p\n",
          curr_th->fetch_data[head].regs_PC, curr_th->fetch_data[head].pred_PC);
...
ruu_fetch(void)
{
  int i, lat, tlb_lat, done[16] = {FALSE};
  md_inst_t inst;
...
/* counter_t itlb_misses; */
      ...
      i++)
  {
        ...
        && !(curr_th->fetch_regs_PC & (sizeof(md_inst_t)-1)))
    {
      ...
      MD_FETCH_INST(inst, curr_th->mem, curr_th->fetch_regs_PC);
      ...
      if (cache_il1)
      {
        ...
        lat =
          cache_access(cache_il1, Read, IACOMPRESS(curr_th->fetch_regs_PC),
      ...
      if (itlb)
      {
        ...
        tlb_lat =
          cache_access(itlb, Read, IACOMPRESS(curr_th->fetch_regs_PC),
              NULL, ISCOMPRESS(sizeof(md_inst_t)), sim_cycle,
              NULL, NULL, th_id);
        if (tlb_lat > 1) {
    ...
    if (pred)
    {
      ...
      if (MD_OP_FLAGS(op) & F_CTRL) {
        curr_th->fetch_pred_PC =
          bpred_lookup(pred,
              /* branch address */curr_th->fetch_regs_PC,
              /* target address *//* FIXME: not computed */0,
              /* opcode */op,
              /* call? */MD_IS_CALL(op),
              /* return? */MD_IS_RETURN(op),
              /* updt */&(curr_th->fetch_data[curr_th->fetch_tail].dir_update),
              /* RSB index */&stack_recover_idx);
      }
      else
        curr_th->fetch_pred_PC = 0;

      /* valid address returned from branch predictor? */
      if (!curr_th->fetch_pred_PC)
      {
        /* no predicted taken target, attempt not taken target */
        curr_th->fetch_pred_PC = curr_th->fetch_regs_PC + sizeof(md_inst_t);
      }
      else
      {
        /* go with target, NOTE: discontinuous fetch, so terminate */
        branch_cnt[th_id]++;
        if (branch_cnt[th_id] >= fetch_speed)
          done[th_id] = TRUE;
      }
    }
    else
    ...
    curr_th->fetch_data[curr_th->fetch_tail].IR = inst;
    curr_th->fetch_data[curr_th->fetch_tail].regs_PC = curr_th->fetch_regs_PC;
    curr_th->fetch_data[curr_th->fetch_tail].pred_PC = curr_th->fetch_pred_PC;
    curr_th->fetch_data[curr_th->fetch_tail].stack_recover_idx = stack_recover_idx;
    curr_th->fetch_data[curr_th->fetch_tail].ptrace_seq = ptrace_seq++;

    /* for pipe trace */
    ptrace_newinst(curr_th->fetch_data[curr_th->fetch_tail].ptrace_seq,
        inst, curr_th->fetch_data[curr_th->fetch_tail].regs_PC,
        0);
    ptrace_newstage(curr_th->fetch_data[curr_th->fetch_tail].ptrace_seq,
        PST_IFETCH,
...
sim_main(void)
{
  ...
  for (th=0; th<nthread; ++th) {
    ...
    /* check for DLite debugger entry condition */
    if (dlite_check_break(curr_th->regs.regs_PC, /* no access */0, /* addr */0, 0, 0))
      dlite_main(curr_th->regs.regs_PC, curr_th->regs.regs_PC + sizeof(md_inst_t),
          sim_cycle, &curr_th->regs, curr_th->mem);
  }
...
#ifdef TARGET_ALPHA
        curr_th->regs.regs_F.d[MD_REG_ZERO] = 0.0;
#endif /* TARGET_ALPHA */

        /* get the next instruction to execute */
        MD_FETCH_INST(inst, curr_th->mem, curr_th->regs.regs_PC);
...
#include "machine.def"
        ...
        if (fault != md_fault_none)
          fatal("fault (%d) detected @ 0x%08p", fault, curr_th->regs.regs_PC);
        ...
        /* check for DLite debugger entry condition */
        if (dlite_check_break(curr_th->regs.regs_NPC,
              is_write ? ACCESS_WRITE : ACCESS_READ,
              addr, sim_num_insn, sim_num_insn))
          dlite_main(curr_th->regs.regs_PC, curr_th->regs.regs_NPC, sim_num_insn, &curr_th->regs, curr_th->mem);
  ...
  for (th=0; th<nthread; ++th) {
    struct thread_t *curr_th = &(thread[th]);
    curr_th->fetch_regs_PC = curr_th->regs.regs_PC - sizeof(md_inst_t);
    curr_th->fetch_pred_PC = curr_th->regs.regs_PC;
    curr_th->regs.regs_PC = curr_th->regs.regs_PC - sizeof(md_inst_t);
  }
