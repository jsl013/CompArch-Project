...
sim_reg_options(struct opt_odb_t *odb)	/* options database */
{
  ...
  opt_reg_string(odb, "-cache:dl1",
      "l1 data cache config, i.e., {<config>|none}",
      &cache_dl1_opt, "dl1:128:32:8:l", /* print */TRUE, NULL);
  opt_reg_note(odb,
  ...
  opt_reg_string(odb, "-cache:il1",
      "l1 inst cache config, i.e., {<config>|dl1|dl2|none}",
      &cache_il1_opt, "il1:512:32:1:l", /* print */TRUE, NULL);
  opt_reg_note(odb,
...
#include "machine.def"
  ...
  double dl1_hit_rate = cache_dl1->hits/(cache_dl1->hits+cache_dl1->misses);
  printf("L1 D-cache hit rate :%lf\n", dl1_hit_rate);
  printf("L1 D-cache hits :%lld\n", cache_dl1->hits);
