...
bp_class_str(int _class)				/* breakpoint class mask */
{
  if (_class == (ACCESS_READ|ACCESS_WRITE|ACCESS_EXEC))
    return "read|write|exec";
  else if (_class == (ACCESS_READ|ACCESS_WRITE))
    return "read|write";
  else if (_class == (ACCESS_WRITE|ACCESS_EXEC))
    return "write|exec";
  else if (_class == (ACCESS_READ|ACCESS_EXEC))
    return "read|exec";
  else if (_class == ACCESS_READ)
    return "read";
  else if (_class == ACCESS_WRITE)
    return "write";
  else if (_class == ACCESS_EXEC)
    return "exec";
  else
...
static char *					/* err str, NULL for no err */
set_break(int _class,				/* break class, use ACCESS_* */
