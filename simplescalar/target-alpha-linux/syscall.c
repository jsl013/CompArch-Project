...
/*	if (regs->regs_R[R_A0] == 0)
        {
          return_value = mem->_system.brk_point;
	  ...
	  success = true;
        ...
        {
          //ld_brk_point
          mem->_system.brk_point = regs->regs_R[R_A0];
          return_value = mem->_system.brk_point;
          ld_brk_point = return_value;
          success = true;
        }*/
...
//          warn("mmap ignorning suggested map address");
          ...
          addr = mem->_system.mmap_end;
          mem->_system.mmap_end += length;
...
//        memory->bcopy(true, vaddr, &buf, sizeof(buf));
	...
	if (new_length > old_length) {
		if ((addr + old_length) == mem->_system.mmap_end) {
			unsigned long int diff = new_length - old_length;
			mem->_system.mmap_end += diff;
			return_value = addr;
			success = true;
			return;
		} else {
//			if (!(flags & 1)) {
		 	...
		 	mem_bcopy(mem_fn, mem, Write,
		  		/*buf*/mem->_system.mmap_end, buf, /*nread*/old_length);
			return_value = mem->_system.mmap_end;
			mem->_system.mmap_end += new_length;			
