...
#define MD_VALID_ADDR(ADDR)						\
  (((ADDR) >= curr_th->mem->ld_text_base && (ADDR) < (curr_th->mem->ld_text_base + curr_th->mem->ld_text_size))	\
   || ((ADDR) >= curr_th->mem->ld_data_base && (ADDR) < curr_th->mem->ld_brk_point)			\
   || ((ADDR) >= (curr_th->mem->ld_stack_base - 16*1024*1024) && (ADDR) < curr_th->mem->ld_stack_base))
